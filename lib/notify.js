...
async function bookingNotify({ guestName, guestEmail, note, startUtc, endUtc, typeLabel, tokenLabel, meetLink, oldStartUtc }) {
  if (!CONFIG.telegramBotToken || !CONFIG.telegramChatId) return;
  const when = fmtWhen(startUtc);
  const mins = Math.round((Date.parse(endUtc) - Date.parse(startUtc)) / 60_000);
  const lines = [
    `${oldStartUtc ? "♻️ Rescheduled" : "🗓"} ${typeLabel || (oldStartUtc ? "booking" : "New booking")}: ${guestName}`,
    oldStartUtc ? `${fmtWhen(oldStartUtc)} → ${when} (${mins} min)` : `${when} (${mins} min)`,
