...
function logBooking(b) {
  const res = open().prepare(`
    INSERT INTO bookings (token, typeKey, guestName, guestEmail, note, startUtc, endUtc, ownerDayKey, gcalEventId, meetLink, manageKey, prepGcalEventId, extraEmails, createdAt)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
module.exports = { open, getToken, getPublicToken, createToken, listTokens, setTokenDisabled, getType, createType, listTypes, setTypeDisabled, bookedByDay, cancelBooking, logBooking, listBookings, newManageKey, getBookingByKey, rescheduleBooking, setPrepEventId, setExtraEmails, extraEmailsOf, DB_PATH, RESERVED_KEYS };
