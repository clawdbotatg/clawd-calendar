...
function open() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS event_types (
      ...
      wrapMinutes     INTEGER,           -- owner-only "Wrap up" event this long, right after
  ...
  // Migration for dbs created before wrap-up blocks (the after-buffer twin of prep).
  ensureColumn("event_types", "wrapMinutes", "INTEGER");
  ensureColumn("bookings", "wrapGcalEventId", "TEXT");
  ...
  // Migration for dbs created before extra guests existed. JSON array of
  // additional attendee emails (the primary guest stays in guestEmail).
  ensureColumn("bookings", "extraEmails", "TEXT");
...
function createType(t) {
  open().prepare(`
    INSERT INTO event_types (key, label, durationMin, stepMinutes, window, dailyCap,
      ...
      prepMinutes, wrapMinutes, addMeet, pageTitle, pageSubtitle, pageDescription,
  ...
  `).run(t.key, t.label, t.durationMin ?? null, t.stepMinutes ?? null,
    ...
    t.eventDescription ?? null, t.eventLocation ?? null, t.prepMinutes ?? null, t.wrapMinutes ?? null,
...
function logBooking(b) {
  const res = open().prepare(`
    INSERT INTO bookings (token, typeKey, guestName, guestEmail, note, startUtc, endUtc, ownerDayKey, gcalEventId, meetLink, manageKey, prepGcalEventId, wrapGcalEventId, extraEmails, createdAt)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(b.token, b.typeKey || null, b.guestName, b.guestEmail, b.note || null, b.startUtc, b.endUtc,
    ...
    b.prepGcalEventId || null, b.wrapGcalEventId || null,
    b.extraEmails && b.extraEmails.length ? JSON.stringify(b.extraEmails) : null,
    new Date().toISOString());
...
function setWrapEventId(id, wrapGcalEventId) {
  open().prepare(`UPDATE bookings SET wrapGcalEventId = ? WHERE id = ?`).run(wrapGcalEventId, id);
}

// Change a type's prep/wrap block lengths in place (admin + ops).
function setTypeBlocks(key, { prepMinutes, wrapMinutes }) {
  return open().prepare(`UPDATE event_types SET prepMinutes = ?, wrapMinutes = ? WHERE key = ?`)
    .run(prepMinutes ?? null, wrapMinutes ?? null, key).changes > 0;
}

// Change a type's booking window in place ({days,start,end}; null = inherit
// the env default). Ops: node scripts/types.js --set-window <key> --days ...
function setTypeWindow(key, window) {
  return open().prepare(`UPDATE event_types SET window = ? WHERE key = ?`)
    .run(window ? JSON.stringify(window) : null, key).changes > 0;
}

// Extra invitees (beyond the primary guest), as a JSON array of emails.
function setExtraEmails(id, emails) {
  return open().prepare(`UPDATE bookings SET extraEmails = ? WHERE id = ? AND status = 'booked'`)
    .run(emails && emails.length ? JSON.stringify(emails) : null, id).changes > 0;
}

// bookings.extraEmails (JSON text or NULL) → array, defensively.
function extraEmailsOf(b) {
  try { return JSON.parse(b.extraEmails) || []; } catch { return []; }
}

function listBookings({ upcomingOnly = false } = {}) {
  if (upcomingOnly) {
    return open()
      .prepare(`SELECT * FROM bookings WHERE status='booked' AND startUtc >= ? ORDER BY startUtc`)
      .all(new Date().toISOString());
  }
  return open().prepare(`SELECT * FROM bookings ORDER BY startUtc`).all();
}

module.exports = { open, getToken, getPublicToken, createToken, listTokens, setTokenDisabled, getType, createType, listTypes, setTypeDisabled, bookedByDay, cancelBooking, logBooking, listBookings, newManageKey, getBookingByKey, rescheduleBooking, setPrepEventId, setWrapEventId, setTypeBlocks, setTypeWindow, setExtraEmails, extraEmailsOf, DB_PATH, RESERVED_KEYS };
