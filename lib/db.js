...
function getPublicToken(typeKey = null) {
  const row = open().prepare(
    `SELECT token FROM tokens WHERE isPublic = 1 AND disabled = 0 AND typeKey IS ? ORDER BY createdAt DESC LIMIT 1`
  ).get(typeKey);
...
module.exports = { open, getToken, getPublicToken, createToken, listTokens, setTokenDisabled, getType, createType, listTypes, setTypeDisabled, bookedByDay, cancelBooking, logBooking, listBookings, DB_PATH, RESERVED_KEYS };
