...
// code → { email, name, accessToken } (short-lived; used once, never stored)
async function guestExchange(code, redirectUri) {
  if (FAKE) return { email: "guest@example.com", name: "Guest Example", accessToken: "fake" };
  const c = webCreds();
  const res = await fetch("https://oauth2.googleapis.com/token", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
  ...
  return { email, name, accessToken: j.access_token };
}
