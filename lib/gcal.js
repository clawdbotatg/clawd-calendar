...
async function listEvents(calendarId, timeMinIso, timeMaxIso) {
  ...
  do {
    const qs = new URLSearchParams({
      ...
      fields: "items(id,summary,start),nextPageToken",
    });
...
const GUEST_SCOPE = "openid email https://www.googleapis.com/auth/calendar.freebusy";
...
// code → { email, accessToken } (short-lived; used once, never stored)
async function guestExchange(code, redirectUri) {
  if (FAKE) return { email: "guest@example.com", accessToken: "fake" };
  const c = webCreds();
  const res = await fetch("https://oauth2.googleapis.com/token", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
  ...
  return { email, accessToken: j.access_token };
}
...
module.exports = { freeBusy, listEvents, createEvent, createOwnerEvent, creds, FAKE, webCreds, guestAuthUrl, guestExchange, guestFreeBusy };
