...
// end, day, endDay, free }] (start/end = ISO dateTime; day/endDay =
// YYYY-MM-DD for all-day events, endDay exclusive like Google's; free =
// marked "free"/transparent; declined = the owner declined the invite —
// neither counts as busy). Feeds
...
async function listEvents(calendarId, timeMinIso, timeMaxIso) {
  ...
  do {
    const qs = new URLSearchParams({
      ...
      fields: "items(id,summary,description,start,end,transparency,attendees(self,responseStatus)),nextPageToken",
    });
    ...
    for (const ev of j.items || []) out.push({
      ...
      endDay: (ev.end && ev.end.date) || null,
...
async function createEvent({ calendarId, summary, description, location, startUtc, endUtc, guestEmail, guestName, extraEmails = [], addMeet }) {
  if (FAKE) {
    console.log(`[gcal:fake] createEvent ${JSON.stringify({ summary, location, startUtc, endUtc, guestEmail, extraEmails, addMeet, description })}`);
  ...
  const body = {
    ...
    attendees: [{ email: guestEmail, displayName: guestName }, ...extraEmails.map((email) => ({ email }))],
...
async function createOwnerEvent({ calendarId, summary, description, startUtc, endUtc }) {
  if (FAKE) {
    ...
    return { id: `fake-owner-${Date.now()}-${Math.random().toString(36).slice(2, 6)}` };
  }
  const ev = await api("POST",
...
// Replace an event's attendee list. sendUpdates=all → Google emails newly
// added attendees a real invite (and removed ones a cancellation); the
// event's time, description, and Meet link are untouched. Throws with
// .status = 404/410 when the event is gone, so callers can cancel locally.
async function patchEventAttendees({ calendarId, eventId, attendees, sendUpdates = "all" }) {
  if (FAKE) {
    console.log(`[gcal:fake] patchEventAttendees ${JSON.stringify({ eventId, attendees })}`);
    return { id: eventId };
  }
  const tok = await accessToken();
  const res = await fetch(
...
// Contacts autocomplete is a SECOND, separate grant — requested only when
// the guest clicks the search-my-contacts button on the add-guests UI,
// never at calendar-connect time (keeps the main consent prompt to
// free/busy only). Saved contacts + "other contacts" (the Gmail
// autocomplete pool). Needs the People API enabled on the OAuth project.
const CONTACTS_SCOPE = "https://www.googleapis.com/auth/contacts.readonly https://www.googleapis.com/auth/contacts.other.readonly";

function guestAuthUrl(redirectUri, state, scope = GUEST_SCOPE) {
  const c = webCreds();
  if (!c) return null;
...
module.exports = { freeBusy, listEvents, createEvent, createOwnerEvent, patchEventTime, patchEventAttendees, deleteEvent, creds, FAKE, webCreds, guestAuthUrl, guestExchange, guestFreeBusy, CONTACTS_SCOPE };
