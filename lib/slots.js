...
// Days already holding a calendar event NAMED like this type's event —
// case-insensitive contains, "Prepare: …" companion blocks excluded — so a
// manually-added "SLOP.COMPUTER" spends its day and deleting an episode
// frees it. Returns null (= no signal) when the title has no fixed name to
// match ({name} templates) or event data is unavailable.
function titledDayCounts(events, eventTitle, tz) {
  ...
  for (const ev of events) {
    ...
    if (!s.includes(needle) || s.startsWith("prepare")) continue;
...
module.exports = { getOpenSlots, effectiveRules, applyType, titledDayCounts };
