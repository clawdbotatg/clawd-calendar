...
// meeting someone added on top later) gets its busy time back, so the
// guest can't be moved over it. `events` null (no event data) = plain carve.
function excludeBookingBusy(busy, events, booking, cfg) {
  ...
  for (const ev of events) {
    if (!ev.start || !ev.end || ev.free || own.has(ev.id)) continue;
    if (companionMatches(ev, "prep", cfg, booking) || companionMatches(ev, "wrap", cfg, booking)) continue;
    const s = Math.max(toMs(ev.start), cutStart), e = Math.min(toMs(ev.end), cutEnd);
    if (e > s) out.push({ start: new Date(s).toISOString(), end: new Date(e).toISOString() });
  }
  return out;
}
