...
function applyType(config, type) {
  ...
  return {
    ...
    // Same for the wrap-up block after: the slot must END wrapMinutes clear
    // of the next busy event, so the block never lands on top of it.
    wrapMinutes: type.wrapMinutes || 0,
    bufferAfterMinutes: Math.max(config.bufferAfterMinutes || 0, type.wrapMinutes || 0),
...
// / "Wrap up: …" companion blocks excluded — so a manually-added episode spends its day
...
function titledDayCounts(events, eventTitle, tz) {
  ...
  for (const ev of events) {
    ...
    if (!hit || s.startsWith("prepare") || s.startsWith("wrap up")) continue;
...
// ── companion blocks (prep before / wrap after a booking) ─────────────────

// Where a booking's companion block sits, as [startMs, endMs].
function companionSpan(kind, cfg, startUtc, endUtc) {
  if (kind === "prep") return [toMs(startUtc) - (cfg.prepMinutes || 0) * 60_000, toMs(startUtc)];
  if (kind === "wrap") return [toMs(endUtc), toMs(endUtc) + (cfg.wrapMinutes || 0) * 60_000];
  throw new Error(`unknown companion kind ${kind}`);
}
const COMPANION_PREFIX = { prep: "Prepare:", wrap: "Wrap up:" };

// Is this calendar event the booking's companion block of that kind? Used
// for legacy bookings with no stored id: the summary prefix + exact start
// alone would also match another guest's block at the same instant, so the
// description must name this guest's email too (every companion we create
// carries "with <name> <email>").
function companionMatches(ev, kind, cfg, booking) {
  if (!ev || !ev.start || !String(ev.summary || "").startsWith(COMPANION_PREFIX[kind])) return false;
  const [s] = companionSpan(kind, cfg, booking.startUtc, booking.endUtc);
  if (toMs(ev.start) !== s) return false;
  return String(ev.description || "").includes(`<${booking.guestEmail}>`);
}

// Busy list with a rescheduling guest's OWN footprint removed: their event
// plus its prep/wrap blocks, and nothing else. [start−prep, end+wrap] is
// carved, then every other event that actually sits inside that range
// (another meeting right after a legacy booking that has no wrap block; a
// meeting someone added on top later; an all-day vacation marked busy)
// gets its busy time back, so the guest can't be moved over it. Restored
// time is CLIPPED to what free/busy originally reported, and events the
// owner declined or marked free are skipped outright (they never counted
// as busy, and inside the guest's own slot the clip alone can't tell).
// The booking's own companion blocks are also carved by their REAL span on
// the calendar (by stored id): if the type's prep/wrap changed since the
// booking, the old block is still there until the move deletes/resizes it,
// and must not fence the guest out of its time.
// `events` null (no event data) = plain carve.
function excludeBookingBusy(busy, events, booking, cfg) {
  const cuts = [[
    toMs(booking.startUtc) - (cfg.prepMinutes || 0) * 60_000,
    toMs(booking.endUtc) + (cfg.wrapMinutes || 0) * 60_000,
  ]];
  const own = new Set([booking.gcalEventId, booking.prepGcalEventId, booking.wrapGcalEventId].filter(Boolean));
  const isOwn = (ev) => own.has(ev.id) ||
    companionMatches(ev, "prep", cfg, booking) || companionMatches(ev, "wrap", cfg, booking);
  if (events) for (const ev of events) {
    const span = isOwn(ev) && eventSpan(ev, cfg.ownerTz);
  ...
  for (const c of cuts) {
    ...
    if (last && c[0] <= last[1]) last[1] = Math.max(last[1], c[1]); else merged.push([...c]);
  }
  let out = busy || [];
  for (const [s, e] of merged) out = subtractBusy(out, s, e);
  if (!events) return out;
  const orig = (busy || []).map((b) => [toMs(b.start), toMs(b.end)]);
  for (const ev of events) {
    if (ev.free || ev.declined || isOwn(ev)) continue;
    const span = eventSpan(ev, cfg.ownerTz);
    if (!span) continue;
    for (const [cutStart, cutEnd] of merged) {
      const s = Math.max(span[0], cutStart), e = Math.min(span[1], cutEnd);
      if (e <= s) continue;
      for (const [bs, be] of orig) {
        const cs = Math.max(s, bs), ce = Math.min(e, be);
        if (ce > cs) out.push({ start: new Date(cs).toISOString(), end: new Date(ce).toISOString() });
      }
    }
  }
  return out;
...
// [startMs, endMs] of a listEvents row: timed events as-is, all-day events
// as midnight→midnight in the owner's zone (endDay exclusive, like
// Google's). null when the row has no usable span.
function eventSpan(ev, tz) {
  if (ev.start && ev.end) return [toMs(ev.start), toMs(ev.end)];
  if (ev.day) {
    const midnight = (d) => { const [y, m, dd] = d.split("-").map(Number); return zonedToUtc(y, m, dd, 0, 0, tz); };
    const s = midnight(ev.day);
    const e = ev.endDay ? midnight(ev.endDay) : s + 86_400_000;
    return e > s ? [s, e] : null;
  }
  return null;
}
...
module.exports = { getOpenSlots, effectiveRules, applyType, titledDayCounts, subtractBusy, companionSpan, companionMatches, excludeBookingBusy };
