...
function excludeBookingBusy(busy, events, booking, cfg) {
  const cutStart = toMs(booking.startUtc) - (cfg.prepMinutes || 0) * 60_000;
  const cutEnd = toMs(booking.endUtc) + (cfg.wrapMinutes || 0) * 60_000;
  const out = subtractBusy(busy, cutStart, cutEnd);
  ...
  for (const ev of events) {
    if (ev.free || ev.declined || own.has(ev.id)) continue;
    if (companionMatches(ev, "prep", cfg, booking) || companionMatches(ev, "wrap", cfg, booking)) continue;
