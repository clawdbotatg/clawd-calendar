...
module.exports = {
  ...
  // "shields down": token to use when someone visits / with no password.
  // Unset (default) = the landing page asks for a password.
  publicToken: env("CAL_PUBLIC_TOKEN", ""),

  // Admin page at /admin/<password> (types, links, bookings).
  // Unset (default) = admin disabled entirely.
  adminPassword: env("CAL_ADMIN_PASSWORD", ""),

  // Telegram ping to the owner on each booking. Both unset = off.
  telegramBotToken: env("CAL_TELEGRAM_BOT_TOKEN", ""),
  telegramChatId: env("CAL_TELEGRAM_CHAT_ID", ""),

  // booking-page info panel (all optional)
  pageTitle: env("CAL_PAGE_TITLE", ""),       // default: "Book a call with <owner>"
  pageSubtitle: env("CAL_PAGE_SUBTITLE", ""), // small line under the title (e.g. email)
